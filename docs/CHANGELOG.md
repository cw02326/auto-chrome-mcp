...
- **도구 스키마·응답의 토큰 절감 2차 (`packages/shared/src/tools.ts` 측).** 스키마 전체 문자
  수는 54,667 에서 54,056 로 줄었다(약 1.1%).
  - `chrome_screenshot` description 에 옛 응답 `note` 문구(이미지가 별도 MCP image 블록으로
    오고, `chrome_computer` 가 `imageScale` 로 좌표를 자동 환산하며, 저장 경로는 응답 필드로
  ...
  - 도구 41개 description 전수 점검에서 "언제 쓰나 / 유사 도구와 차이 / 필수 인자" 를 벗어난
    내부 구현 문구 셋을 더 찾아 뺐다: `chrome_javascript`(CDP 메서드 이름), `chrome_upload_file`
    ("using Chrome DevTools Protocol"), `chrome_handle_dialog`("via CDP"). 나머지 38개는
    설명이 전부 위 세 요소에 해당해 그대로 뒀다(`chrome_read_page` 의 응답 형식 설명은
    출력을 해석하는 데 필요한 계약이라 내부 구현 설명으로 보지 않았다).
  - `chrome_navigate` 의 성공 응답에서 `message` 프로즈 8곳을 뺐다(refresh/history/기존 탭
    재활성/새 창/작업 탭 재사용/작업 창 신규 탭/기존 창 신규 탭/폴백 새 창). `success:true` 와
    이미 있던 구조화 필드로 충분한 경우는 그냥 뺐고, 정보가 오직 message 에만 있던 두 곳은
    구조화 필드로 옮겼다(기존 탭을 갱신했는지 활성화만 했는지 `navigatedExplicitTab`, 전용
    MCP 작업 창인지 현재 창인지 `dedicated`). 실패·경고 메시지는 손대지 않았다.
...
- **팝업 뒤 복귀가 창별 활성 탭 기준으로 바뀌었다.** 크롬의 탭 활성화 이벤트에는 이전 탭 정보가
  없어 전역 값에 기대던 것을 창마다 마지막 활성 탭을 기록하는 방식으로 고쳤다. 같은 창에서 열린 탭은
  창 포커스를 건드리지 않고, 팝업 창일 때만 직전 포커스 창으로 돌아간다.
- **흐름 실행 도구는 게이트 단계부터 강제 background 모드로 판정한다.** 전역 토글이 꺼져 있고
  tabId 를 생략해도 작업 탭이 주입되고, actions 경로와 하위 흐름에도 모드·마감이 전달된다.
- **예약 잠금 연산(획득·heartbeat·해제)을 직렬화해 해제 뒤 옛 잠금이 되살아나지 않는다.**
- **이미 발행된 흐름 스냅샷의 민감 변수 기본값을 워커 초기화 때 한 번 정리한다.**
...
- **흐름의 actions 경로에서 나가던 도구 호출이 세션·레인 정보를 잃던 문제를 고쳤다.**
  ...
  토큰을 한꺼번에 싣는다.
- **취소가 대기 한가운데서는 듣지 않던 문제를 고쳤다.** 흐름 엔진은 스텝 경계에서만 취소를
  확인했다. 그래서 60초짜리 `wait { sleep }` 이나 `delay` 안에 들어간 실행은 취소를 받고도
  그 60초가 다 지나야 멈췄고, `record_replay_flow_run` 의 `timeoutMs` 는 응답만 끊고 실행은
  계속 도는 좀비 run 을 남겼다. 이제 엔진의 모든 대기(sleep·delay·재시도 백오프·이동 정찰
  대기)가 취소 신호를 함께 본다. 취소되면 타이머를 지우고 즉시 `run_aborted` 로 닫는다.
- **취소된 실행이 자기가 연 탭을 남기던 문제를 고쳤다.** 스텝 한가운데서 취소되면 그 스텝이
  실패로 닫히면서 순회가 거기서 끝나, 취소 표시를 세우는 지점을 다시 지나지 않았다. 그래서
  소유 탭 정리가 건너뛰어졌다. 이제 정리 여부는 취소 신호를 직접 보고 판단한다. 새 탭 모드
  (`tabTarget` 이 `new`)로 도구가 연 탭도 run 소유로 등록해 같은 정리에 걸린다.
- **트리거 자동 실행이 남기는 탭 정리를 고쳤다.** 정리 대상이 실행 컨텍스트의 현재 탭이라,
  흐름이 도중에 자기가 연 탭으로 옮겨가면 정리는 엉뚱하게 그 새 탭을 닫고 정작 자기가 만든
  부트스트랩 탭과 재귀 가드 항목은 그대로 남겼다. 이제 부트스트랩 탭 id 를 생성 직후에
  붙잡아 두고, run 이 실행 중 연 탭까지 함께 닫는다.
- **스크린샷 노드가 크기를 재려고 이미지 바이트를 통째로 돌려받던 것을 고쳤다.** 파일 크기
  하나 때문에 base64 문자열(장당 수 MB)을 확장 내부로 나른 뒤 곧바로 버렸다. 이제
  `chrome_screenshot` 응답이 저장된 파일 크기를 `bytes` 로 싣고, 노드는 그 값을 쓴다. 옛
  응답처럼 `bytes` 가 없으면 `chrome.downloads` 로 파일 크기를 조회한다.
- **예약 실행 중 페이지가 띄운 탭·팝업 창이 아무 데도 속하지 않던 문제를 고쳤다.** 저장된
  step 의 클릭이 `target=_blank` 나 `window.open` 을 일으키면 그 탭은 소유 버킷 밖이라
  실행이 끝나도 남았고, 활성 탭이 되어 사용자가 보던 화면을 가져갔다. 완화 로직은 전역
  무간섭 토글만 보고 있어서 토글을 끈 크롬에서는 아예 돌지 않았다. 이제 실행이 도는 동안
  그 세션이 연 탭은 곧바로 같은 버킷 소유가 되고, 사용자가 보던 탭·창을 즉시 되돌리며,
  실행이 끝나면 탭과 팝업 창을 함께 닫는다. 팝업 창의 탭은 언제나 그 창의 활성 탭이므로
  인계(`user_took_over_tab`) 판정에서는 제외한다.
- **`chrome_gif_recorder` 가 실행 컨텍스트 모드를 건너뛰던 것을 고쳤다.** 배경 탭 녹화를
  위해 탭을 활성화할지 정할 때 전역 토글만 봐서, 토글이 꺼진 크롬에서 예약 실행이 GIF 를
  찍으면 사용자가 보던 탭이 바뀌었다. 이제 이 호출의 실행 컨텍스트를 먼저 읽고, 무간섭이
  강제된 실행에서는 활성화도 복원도 하지 않는다(정지 화면이 될 수 있다는 경고만 남긴다).
- **지웠다 다시 건 예약을 옛 실행이 자기 것으로 착각하던 문제를 고쳤다.** 판정 기준이던
  `revision` 은 예약마다 1 부터 다시 세므로, 실행 중 `unschedule` 후 같은 이름으로 다시
  걸면 값이 옛 것과 같아졌다(ABA). 그러면 끝난 실행이 남의 예약 상태를 덮어쓰고 알람까지
  다시 걸었다. 이제 예약 저장소 전체에서 한 번 쓴 값을 다시 쓰지 않는 `generation` 으로
  판정하고, 상태 갱신은 CAS 로만 하며 알람은 저장에 성공한 레코드에서만 건다.
- **실행 잠금과 이력 claim 이 갈라져 있던 것을 한 임계 구역으로 합쳤다.** 잠금이 "읽어서
  비어 있으면 쓴다" 라 두 워커가 같은 순간에 읽으면 둘 다 잡았고, shortcut 정의를 읽는
  동안 상대도 "이력에 아직 없다" 를 보고 함께 실행했다. 이제 쓰고 나서 다시 읽어 자기
  nonce 가 남아 있을 때만 잡은 것으로 보고(fenced), 그 자리에서 이력 claim 과 `running`
  기록까지 끝낸다. `reconcile` 은 하트비트가 살아 있는 남의 실행을 `interrupted` 로
  바꾸지 않고 그 버킷의 탭도 건드리지 않는다.
- **120초 상한이 실행을 실제로 끊지 못하던 것을 고쳤다.** 예전에는 상한이 지나면 결과만
  돌려주고 실행은 그대로 살아 있어, 매달린 도구가 끝나는 대로 취소된 실행이 탭을 더 열었다.
  이제 러너에 마감 시각과 취소 신호를 함께 넘겨 남은 step 을 부르지 않게 하고, 러너가 멈춘
  것을 확인한 뒤 정리한다. 잠금 하트비트와 keepalive 도 스크린샷·report·탭 정리까지 살아
  있다(예전에는 그 전에 꺼져 마무리 작업이 잠금 없이 돌았다).
- **마지막 step 뒤의 탭 인계를 확인하지 않던 것을 고쳤다.** 인계 검사가 step 을 부르기
  직전에만 있어서, 마지막 step 이 끝난 뒤 사용자가 그 탭을 열면 실행은 `success` 로
  기록되고 그 탭에 스크린샷·report 를 남기고 탭을 닫았다. 이제 산출물과 정리보다 먼저 한
  번 더 확인해 `user_took_over_tab` 으로 끝내고 산출물을 만들지 않는다.
- **워커가 죽어 `interrupted` 가 된 실행이 예약 상태에 반영되지 않던 것을 고쳤다.** 이력만
  바뀌고 `lastStatus`·`failStreak` 는 그대로라, 밤새 한 번도 못 돈 예약이 아침 `schedules`
  응답에서 "마지막 실행 성공" 으로 보였고 알림도 없었다. 이제 되돌린 실행마다 한 번씩
  상태를 갱신하고 첫 실패 알림을 보낸다(그 사이 예약이 새로 걸렸으면 건드리지 않는다).
- **DST fall-back 날에 두 번 오는 시각을 뒤엣것으로 잡던 계산을 고쳤다.** 벽시계가 01:59
  다음에 01:00 으로 되돌아가는 날은 "목표 시각 이상" 조건이 참·거짓·참으로 갈리는데,
  이분 탐색이 그 전제를 어겨 표준시로 넘어간 쪽을 골랐다(`America/New_York` 의 01:45 에서
  드러난다). 이제 실제 순간을 분 단위로 훑어 첫 발생을 쓴다.
- **이력 저장이 실패하면 이유를 가리지 않고 보관량을 절반으로 자르던 것을 고쳤다.** 확장
  컨텍스트 무효화처럼 용량과 무관한 오류에도 밤새 쌓인 기록의 절반이 사라졌다. 이제 용량
  초과로 보일 때만, 가장 오래된 1건씩 최대 3회 지우고 다시 시도한다(방금 쓰는 기록은
  지우지 않는다). 다른 오류는 그대로 알린다.
- **`report: true` 가 약속한 256KiB 를 실제로 담지 못하던 것을 고쳤다.** 이력용 결과가
  24,000자에서 잘린 뒤에 report 파일을 만들어, 상한을 아무리 크게 잡아도 그보다 큰 값은
...
- **흐름 실행 중 다른 세션이 그 탭에 끼어들 수 있던 구멍을 막았다.** 실행이 작업 탭에 리스를
  걸어도, 도구 파이프라인이 리스 토큰을 잠금에 넘기지 않아 차단 모드를 켤 수 없었다(켜면
  실행이 자기 리스에 막혀 첫 스텝에서 교착했다). 이제 파이프라인이 내부 인자로 실려 온
  토큰을 잠금에 넘기고 도구에는 넘기지 않으며, 토큰 없는 바깥 호출은 리스가 풀릴 때까지
  기다린다. 리스 보유자 기록도 단일 값에서 스택으로 바꿔, 겹친 리스가 역순으로 끝날 때
  아무도 풀어 주지 않는 유령 보유자가 남던 것을 없앴다.
- **리스가 탭 이동을 따라가지 않던 문제를 막았다.** 리스는 실행을 시작한 탭에만 걸려 있어,
  흐름이 자기가 연 탭으로 옮겨가는 순간 그 탭에는 아무 잠금도 없었다. 스텝과 스텝 사이에
  다른 세션이 그 탭을 잡을 수 있었다. 이제 실행이 옮겨가거나 새로 여는 탭마다 같은 토큰으로
  리스를 걸고, 실행이 끝나면 전부 푼다.
- **발행 경계를 우회할 수 있던 두 경로를 막았다.** ① 실행 대상 흐름을 id 로 먼저 찾고 발행
  목록은 허가 확인에만 썼기 때문에, 어떤 흐름의 slug 가 발행되지 않은 다른 흐름의 id 와
  겹치면 그 초안이 남의 허가로 실행됐다. ② 발행은 메타데이터만 저장하고 실행 시점에 그때의
  초안을 다시 읽어, 발행한 뒤 초안을 고치면(편집은 버전을 올리지 않는다) 승인받지 않은
  내용이 그대로 돌았다. 이제 대상은 발행 레코드로만 정하고, 실행되는 내용은 발행 시점에
  함께 저장한 스냅샷이다. 스냅샷 이전에 발행된 레코드는 저장된 흐름의 버전이 발행 당시와
  같을 때만 실행하고, 다르면 `flow_version_mismatch` 로 거절한다.
  `record_replay_list_published` 응답에는 스냅샷을 싣지 않는다.
