...
**Scope:** auto-chrome-mcp 사용자가 troubleshooting 문제를 Claude Code 에 자연어로 말하면 자동으로 `doctor` / `--fix` / `report` 를 호출·해석하고 설정 문제 (CHROME_PORT, 강제포커스) 도 다뤄주는 SKILL.md 를 패키지에 박제 + welcome 페이지의 prompt 박스로 설치 동의 받기.
...
- 사용자 글로벌 install 후 경로: `<npm root -g>/auto-chrome-mcp-bridge/dist/skill/SKILL.md`
...
│ │ auto-chrome-mcp-doctor/SKILL.md 로               │  │
│ │ 복사해서 스킬로 등록해줘:                       │  │
│ │   <npm root -g>/auto-chrome-mcp-bridge/          │  │
│ │   dist/skill/SKILL.md                           │  │
...
2. `~/.claude/skills/auto-chrome-mcp-doctor/` 디렉토리 생성 (`mkdir -p`)
3. 기존 SKILL.md 존재 시 `head -10` 으로 frontmatter 확인:
   - 같은 name + auto-chrome-mcp-version 차이 → diff 보여주고 overwrite 동의
...
```yaml
---
name: auto-chrome-mcp-doctor
description: |
  Use whenever the user reports ANY problem with the auto-chrome-mcp
...
# auto-chrome-mcp-version: <bridge version at build time>
---
```
...
| 증상                                 | 1st 명령                              | 후속                                          |
| ------------------------------------ | ------------------------------------- | --------------------------------------------- |
| 연결 안 됨 / disconnected            | `auto-chrome-mcp-bridge doctor`       | 빨간불 항목 보고 `--fix`                      |
...
- description 첫 줄에 "auto-chrome-mcp" 패키지명 명시
- "Skip if upstream hangwin/mcp-chrome" line → 다른 fork 사용자에게 false trigger 안 함
- 본문 (A) Architecture 섹션 첫 줄에 "This skill assumes you're on the auto-chrome-mcp fork — verify with `auto-chrome-mcp-bridge -V`"

### 2.4 갱신 정책 (사용자 dirty 보호)

- `# auto-chrome-mcp-version: 1.0.28` frontmatter comment 박제 (build 시 자동 주입)
...
2. `app/native-server/src/scripts/build.ts` — `dist/skill/SKILL.md` 복사 + frontmatter 의 `# auto-chrome-mcp-version:` 을 package.json version 으로 치환
...
- **전체 사용 가이드 (chrome_screenshot 등 도구 사용법까지) 포함**: YAGNI. 한 스킬에 너무 많은 책임 — 필요시 별도 `auto-chrome-mcp-usage` 스킬로 분리 가능.
