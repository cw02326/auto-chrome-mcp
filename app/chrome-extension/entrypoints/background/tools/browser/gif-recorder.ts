...
async function restoreActiveTab(restoreTab?: RestoreTabInfo): Promise<void> {
  ...
  try {
    ...
    // ignore - tab/window may no longer exist
  }
}
      ...
      switch (action) {
        case 'start': {
          ...
          try {
            ...
            if (tab.windowId !== focusedWindow.id) {
              const [prevActiveTab] = await chrome.tabs.query({
                active: true,
              ...
              if (!activated) {
                ...
                warning =
                  'recording a background tab: activation was blocked by no-interference mode, page animations may be paused';
              }
            } else if (!tab.active) {
              // 포커스된 창의 비활성 탭은 사용자 몰래 전환하지 않고 경고만 남긴다.
              warning =
                'recording a background tab in the focused window: page animations may be paused';
            }
          } catch (error) {
            console.warn(
              'GIF recorder: Failed to check/activate background tab (non-fatal):',
              error,
            );
          }
        ...
        case 'stop': {
          ...
          if (autoTab !== undefined && isAutoCaptureActive(autoTab)) {
            ...
            // Save GIF file
            const blob = new Blob([stopResult.gifData as unknown as BlobPart], {
              type: 'image/gif',
            });
            const dataUrl = await blobToDataUrl(blob);
        ...
        case 'export': {
          ...
          if (download) {
            // Download mode
            const blob = new Blob([lastRecordedGif.gifData as unknown as BlobPart], {
              type: 'image/gif',
            });
            const dataUrl = await blobToDataUrl(blob);
