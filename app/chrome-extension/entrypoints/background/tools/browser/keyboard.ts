...
class KeyboardTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: KeyboardToolParams): Promise<ToolResult> {
    ...
    try {
      ...
      // 헬퍼는 개별 키/텍스트가 전부 실패해도 error 없이 success:false 만 돌려준다.
      // 그대로 통과시키면 호출자에게 성공으로 보이므로 여기서 오류로 승격한다.
      if (result.success === false) {
        const failures = (result.results || [])
          .filter((r: { success?: boolean }) => r && r.success === false)
          .map(
            (r: { keyCombination?: string; error?: string }) =>
              `${r.keyCombination}: ${r.error || 'failed'}`,
          )
          .join('; ');
        return createErrorResponse(failures || result.message || 'Keyboard operation failed');
      }
