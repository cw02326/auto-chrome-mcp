<template>
  <div class="section">
    <h2 class="section-title">{{ getMessage('sidepanel_nav_markers_title') }}</h2>
    <div class="config-card">
      ...
      <form class="mcp-config-section" @submit.prevent="onAdd">
        <div class="mcp-config-header">
          <p class="mcp-config-label">{{ getMessage('sidepanel_marker_add') }}</p>
        </div>
        <div style="display: flex; gap: 8px; margin-bottom: 8px">
          ...
          <select v-model="form.matchType" class="port-input" style="max-width: 120px">
            <option value="prefix">{{ getMessage('sidepanel_match_type_prefix') }}</option>
        ...
        >
          ...
          <div style="display: flex; gap: 6px">
            <button class="semantic-engine-button" @click="validate(m)">
              {{ getMessage('sidepanel_marker_verify') }}
            </button>
            <button class="secondary-button" @click="prefill(m)">
              {{ getMessage('sidepanel_edit_button') }}
            </button>
