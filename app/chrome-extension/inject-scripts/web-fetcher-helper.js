...
if (window.__WEB_FETCHER_HELPER_INITIALIZED__) {
  ...
  /**
   * auto-chrome-mcp fork: htmlContent 기본 상한.
   * 텍스트 경로(maxTotalLength: 100000) 와 같은 눈금으로 맞췄다 — 태그가 섞인 만큼
   * 실제 정보량은 더 적지만, 두 경로가 서로 다른 크기로 튀지 않게 하는 쪽을 택했다.
   */
  const HTML_DEFAULT_MAX_CHARS = 100000;
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    ...
    // Get HTML content
      ...
      try {
        ...
        // auto-chrome-mcp fork: HTML 경로에는 상한이 없어서 무거운 페이지 한 번에 수 MB 가
        // 그대로 모델 컨텍스트로 흘러갔다 (텍스트 경로는 100k 상한 + reader + diff 가 다 있었다).
        // 여기서 잘라 전송량 자체를 줄이고, 잘렸는지와 원문 길이를 함께 알린다.
        const htmlMaxChars =
          typeof request.maxChars === 'number' && request.maxChars > 0
            ? request.maxChars
            : HTML_DEFAULT_MAX_CHARS;
        const fullHtmlChars = cleanedHtml.length;
        const htmlTruncated = fullHtmlChars > htmlMaxChars;

        sendResponse({
          success: true,
          htmlContent: htmlTruncated ? cleanedHtml.slice(0, htmlMaxChars) : cleanedHtml,
          fullHtmlChars,
          returnedChars: htmlTruncated ? htmlMaxChars : fullHtmlChars,
