...
function installChromeMocks(): ChromeHarness {
  ...
  // v1.9.0: 작업 창 표지 검증이 창의 탭 목록(populate)을 본다 — 창마다 첫 탭을 만들어 둔다.
  ...
  const windowsCreate = vi.fn(async (createData: Record<string, unknown>) => {
    ...
    return { id, type: 'normal', tabs: [{ id: tabId }], ...createData };
  });
  ...
  (globalThis as unknown as { chrome: unknown }).chrome = {
    ...
    windows: {
      get: vi.fn(async (windowId: number, info?: { populate?: boolean }) => {
        if (!openWindows.has(windowId)) throw new Error(`No window with id: ${windowId}`);
        const base = { id: windowId, type: 'normal' };
        if (info?.populate) {
          return { ...base, tabs: (windowTabs.get(windowId) ?? []).map((id) => ({ id })) };
        }
        return base;
      }),
...
describe('mcp-window-manager (auto-chrome-mcp fork — 작업 창 모드)', () => {
  ...
  describe('작업 창 수명주기', () => {
    ...
    it('창 id 를 storage.session 에 기록하고 재사용한다', async () => {
      ...
      // v1.9.0: 창 id 만이 아니라 "우리가 만든 창" 표지를 함께 저장한다.
      expect((h.sessionStore.mcpWorkWindowId as { id: number }).id).toBe(first);
