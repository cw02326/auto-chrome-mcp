...
2. Download the matching Chrome extension to `~/Downloads/auto-chrome-mcp-extension-v<version>/`.
...
> Register `chrome-mcp-stdio` in `.mcp.json` with `command: "node"`, `args: ["<npm root -g>/auto-chrome-mcp-bridge/dist/mcp/mcp-server-stdio.js"]`, `env: { "CHROME_PORT": "12320" }`. Run `npm root -g` first to resolve the path. Preserve any existing servers.
...
3. Click **Load unpacked** and select `~/Downloads/auto-chrome-mcp-extension-v<version>/`
...
```bash
auto-chrome-mcp-bridge doctor        # health check
auto-chrome-mcp-bridge doctor --fix  # attempt auto-repair
auto-chrome-mcp-bridge report --copy # copy a diagnostic report to clipboard
```
