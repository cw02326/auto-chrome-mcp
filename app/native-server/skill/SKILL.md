...
# auto-chrome-mcp doctor

이 스킬은 사용자가 auto-chrome-mcp (hangwin/mcp-chrome fork) 의 MCP 서버, 확장,
...
doctor 는 12개 이상 항목 점검(설치된 브라우저 수·OS 에 따라 늘어남). 결과 stdout 에서 ❌ 항목별 결정:
...
| **Chrome / Chromium manifest**      | OS 별 디렉토리에 `com.autochromemcp.nativehost.json` 없음 | `auto-chrome-mcp-bridge doctor --fix` 로 user-level 자동 재등록                                                                                                                                                                                                                                                                                                                                               |
