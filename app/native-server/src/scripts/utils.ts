 ...
 * This ensures the native host uses the same Node.js that launched it,
 * keeping the dev/install Node.js and the runtime Node.js in sync.
 *
...
async function ensureWindowsFilePermissions(packageDistDir: string): Promise<void> {
  ...
  for (const filePath of filesToCheck) {
    if (fs.existsSync(filePath)) {
      try {
        // Check read-only flag; remove if set
...
export async function createManifestContent(): Promise<any> {
  ...
  return {
    ...
    type: 'stdio',
    allowed_origins: [
      // v1.0.2 부터 upstream 호환 제거 — upstream extension 은 com.chromemcp.nativehost 그대로 사용.
...
/**
 * Verify Windows registry key exists and points to correct path
 */
...
/**
 * Try user-level Native Messaging host registration
 */
export async function tryRegisterUserLevelHost(targetBrowsers?: BrowserType[]): Promise<boolean> {
  try {
    ...
    if (browsersToRegister.length === 0) {
      // Default to Chrome and Chromium if none detected
    ...
    for (const browserType of browsersToRegister) {
      ...
      try {
        ...
        // Windows needs additional registry entry
        if (os.platform() === 'win32' && config.registryKey) {
          try {
            // Note: no need to manually escape backslashes — reg handles Windows paths
...
// Import is-admin (Windows-only)
let isAdmin: () => boolean = () => false;
if (process.platform === 'win32') {
  try {
    isAdmin = require('is-admin');
  } catch (error) {
    console.warn('is-admin missing — Windows admin detection may not work');
    console.warn(error);
  }
}

/**
 * Register system-level manifest with elevated privileges
 */
export async function registerWithElevatedPermissions(): Promise<void> {
  try {
    ...
    const hasAdminRights = process.platform === 'win32' ? isAdmin() : false; // Detect admin privileges on Windows
    ...
    if (hasElevatedPermissions) {
      // Already admin → execute commands directly
      try {
        // Create directory
    ...
    // 6. Windows-specific — set system-level registry
    if (os.platform() === 'win32') {
      ...
      if (hasElevatedPermissions) {
        // Already admin → execute registry commands directly
        try {
          execSync(regCommand, { stdio: 'pipe' });
