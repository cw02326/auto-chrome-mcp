...
import { COMMAND_NAME, EXTENSION_ID, HOST_NAME } from './constant';
import { getAllBrowserConfigs } from './browser-config';
import { colorText, tryRegisterUserLevelHost, writeNodePathFile } from './utils';

// auto-chrome-mcp fork: extension zip 자동 다운로드
// - 앱 런타임 상태 (cdp-port 등) 는 ~/.auto-chrome-mcp/ 에 유지
...
const AUTO_CHROME_MCP_INSTALL_ROOT = path.join(os.homedir(), '.auto-chrome-mcp');
...
const AUTO_CHROME_MCP_EXT_DIR = path.join(
  os.homedir(),
  'Downloads',
  `auto-chrome-mcp-extension-v${BRIDGE_VERSION}`,
);
const AUTO_CHROME_MCP_EXT_URL =
  'https://github.com/cw02326/auto-chrome-mcp/releases/latest/download/auto-chrome-mcp-extension.zip';

// 이전 이름 정리용.
const LEGACY_HOST_NAME = 'com.chromemcpscalemaker.nativehost';
// 이전 이름 정리용.
const LEGACY_SKILL_DIR_NAME = 'chrome-mcp-scalemaker-doctor';
// 이전 이름 정리용. 확장 zip 캐시·cdp-port·agent DB 가 들어 있던 폴더.
const LEGACY_INSTALL_ROOT = path.join(os.homedir(), '.mcp-chrome-scalemaker');
...
function removeLegacyAgentDb(): void {
  const legacyDbPaths = [
    path.join(AUTO_CHROME_MCP_INSTALL_ROOT, 'agent.db'),
    path.join(AUTO_CHROME_MCP_INSTALL_ROOT, 'agent.sqlite'),
    path.join(AUTO_CHROME_MCP_INSTALL_ROOT, 'agent.db-journal'),
    path.join(AUTO_CHROME_MCP_INSTALL_ROOT, 'agent.db-wal'),
    path.join(AUTO_CHROME_MCP_INSTALL_ROOT, 'agent.db-shm'),
  ];
...
async function downloadAndExtractExtension(): Promise<string | null> {
  if (process.env.AUTO_CHROME_MCP_SKIP_EXTENSION_DOWNLOAD === '1') {
    console.log(
      colorText(
        'Skipping extension download (AUTO_CHROME_MCP_SKIP_EXTENSION_DOWNLOAD=1)',
        'yellow',
      ),
    );
    return null;
  ...
  fs.mkdirSync(AUTO_CHROME_MCP_INSTALL_ROOT, { recursive: true });
  fs.mkdirSync(path.dirname(AUTO_CHROME_MCP_EXT_DIR), { recursive: true });
  const zipPath = path.join(AUTO_CHROME_MCP_INSTALL_ROOT, 'extension.zip');
  ...
  try {
    if (os.platform() === 'win32') {
      ...
      const psCommand = [
        ...
        `Invoke-WebRequest -UseBasicParsing -Uri "${AUTO_CHROME_MCP_EXT_URL}" -OutFile "${zipPath}"`,
      ].join('; ');
      ...
      await runCmd('curl', ['-fsSL', AUTO_CHROME_MCP_EXT_URL, '-o', zipPath]);
    }
  ...
  // 압축 해제
  try {
    fs.rmSync(AUTO_CHROME_MCP_EXT_DIR, { recursive: true, force: true });
    fs.mkdirSync(AUTO_CHROME_MCP_EXT_DIR, { recursive: true });
    if (os.platform() === 'win32') {
      // Windows 10+ 의 내장 tar 사용 (zip 도 처리 가능)
      await runCmd('tar', ['-xf', zipPath, '-C', AUTO_CHROME_MCP_EXT_DIR]);
    } else {
      await runCmd('unzip', ['-q', '-o', zipPath, '-d', AUTO_CHROME_MCP_EXT_DIR]);
    }
    fs.unlinkSync(zipPath);

    // v1.0.36: 압축 해제 검증 — manifest.json 존재 = chrome extension 의 필수 파일.
    // 빈 폴더 / 손상된 zip / 잘못된 zip 차단.
    const manifestPath = path.join(AUTO_CHROME_MCP_EXT_DIR, 'manifest.json');
    if (!fs.existsSync(manifestPath)) {
      const fileCount = fs.readdirSync(AUTO_CHROME_MCP_EXT_DIR).length;
      try {
        fs.rmSync(AUTO_CHROME_MCP_EXT_DIR, { recursive: true, force: true });
      } catch {
        /* ignore */
    ...
    try {
      const versionLine = `auto-chrome-mcp v${BRIDGE_VERSION}\n`;
      const buildLine = `Installed: ${new Date().toISOString()}\n`;
      const releaseLine = `Release: https://github.com/cw02326/auto-chrome-mcp/releases/tag/v${BRIDGE_VERSION}\n`;
      const extIdLine = `Extension ID: ${EXTENSION_ID}\n`;
      fs.writeFileSync(
        path.join(AUTO_CHROME_MCP_EXT_DIR, 'VERSION'),
    ...
    console.log(colorText(`✓ Extension extracted to ${AUTO_CHROME_MCP_EXT_DIR}`, 'green'));
    return AUTO_CHROME_MCP_EXT_DIR;
  } catch (e) {
...
function printDownloadFailureGuide(errorMessage: string): void {
  ...
  console.log(colorText(`      ${AUTO_CHROME_MCP_EXT_URL}`, 'blue'));
  console.log(`  [2] 다운받은 zip 을 다음 폴더에 압축 해제:`);
  console.log(colorText(`      ${AUTO_CHROME_MCP_EXT_DIR}`, 'green'));
  console.log(`  [3] chrome://extensions 열기 → Developer mode ON → "Load unpacked"`);
  console.log(`      → [2] 의 폴더 선택`);
}

function printNextSteps(extDir: string | null): void {
  ...
  if (extDir) {
    ...
    console.log(`    https://github.com/cw02326/auto-chrome-mcp/releases/latest`);
  }
 ...
 * - AUTO_CHROME_MCP_NO_SKILL=1 env 면 opt-out
 * - 이미 같은 name (auto-chrome-mcp-doctor) 의 SKILL.md 있으면 auto-chrome-mcp-version
 *   비교 — 같으면 skip, 다르면 조용히 overwrite
 * - 다른 name (사용자 custom 스킬) 이 같은 경로면 절대 덮지 않음
 */
function installClaudeCodeSkill(): void {
  if (process.env.AUTO_CHROME_MCP_NO_SKILL === '1') {
    console.log(colorText('[skill] AUTO_CHROME_MCP_NO_SKILL=1 — skipping skill install', 'yellow'));
    return;
  }
  ...
  const skillDir = path.join(claudeRoot, 'skills', 'auto-chrome-mcp-doctor');
  ...
  if (fs.existsSync(skillDest)) {
    try {
      ...
      if (nameMatch && nameMatch[1] !== 'auto-chrome-mcp-doctor') {
        console.log(
          colorText(
      ...
      const existingVersion = existing.match(/^# auto-chrome-mcp-version:\s*(\S+)/m)?.[1];
      const newSource = fs.readFileSync(skillSource, 'utf8');
      const newVersion = newSource.match(/^# auto-chrome-mcp-version:\s*(\S+)/m)?.[1];
...
/**
 * v1.10.0: 이전 브랜딩으로 남은 설치 흔적 정리.
 * best-effort 로 동작한다. 실패해도 설치는 계속하고 경고만 출력한다.
 * 새 스킬을 설치한 뒤에 호출해야 한다. 새 SKILL.md 가 실제로 있을 때만 이전 스킬을
 * 지운다 (새 스킬 설치가 실패하거나 opt-out 된 경우 사용자가 아무 스킬도 갖지 못하는
 * 상황을 피한다).
 */
function cleanupLegacyInstall(): void {
  const skillsRoot = path.join(os.homedir(), '.claude', 'skills');

  // 1. 이전 이름의 Claude Code 스킬 디렉터리 (새 스킬이 자리 잡은 뒤에만)
  try {
    const legacySkillDir = path.join(skillsRoot, LEGACY_SKILL_DIR_NAME);
    const newSkillFile = path.join(skillsRoot, 'auto-chrome-mcp-doctor', 'SKILL.md');
    if (fs.existsSync(legacySkillDir) && fs.existsSync(newSkillFile)) {
      fs.rmSync(legacySkillDir, { recursive: true, force: true });
      console.log(colorText(`✓ [migrate] 이전 스킬 삭제: ${legacySkillDir}`, 'green'));
    }
  } catch (e: any) {
    console.warn(colorText(`[migrate] 이전 스킬 삭제 실패 (무시): ${e.message}`, 'yellow'));
  }

  // 2. 이전 설치 루트 (extension.zip 캐시, cdp-port, 옛 agent DB). 사용자 데이터는 없다.
  try {
    if (fs.existsSync(LEGACY_INSTALL_ROOT)) {
      fs.rmSync(LEGACY_INSTALL_ROOT, { recursive: true, force: true });
      console.log(colorText(`✓ [migrate] 이전 설치 폴더 삭제: ${LEGACY_INSTALL_ROOT}`, 'green'));
    }
  } catch (e: any) {
    console.warn(colorText(`[migrate] 이전 설치 폴더 삭제 실패 (무시): ${e.message}`, 'yellow'));
  }

  // 3. 이전 이름의 네이티브 호스트 manifest (+ Windows 는 HKCU 레지스트리 키까지)
  let browserConfigs: ReturnType<typeof getAllBrowserConfigs> = [];
  try {
    browserConfigs = getAllBrowserConfigs();
  } catch (e: any) {
    console.warn(colorText(`[migrate] 브라우저 설정 조회 실패 (무시): ${e.message}`, 'yellow'));
  }
  for (const config of browserConfigs) {
    try {
      const legacyManifest = path.join(
        path.dirname(config.userManifestPath),
        `${LEGACY_HOST_NAME}.json`,
      );
      if (fs.existsSync(legacyManifest)) {
        fs.rmSync(legacyManifest, { force: true });
        console.log(colorText(`✓ [migrate] 이전 manifest 삭제: ${legacyManifest}`, 'green'));
      }
    } catch (e: any) {
      console.warn(colorText(`[migrate] 이전 manifest 삭제 실패 (무시): ${e.message}`, 'yellow'));
    }

    if (os.platform() !== 'win32' || !config.registryKey) continue;
    const legacyKey = config.registryKey.replace(HOST_NAME, LEGACY_HOST_NAME);
    try {
      execSync(`reg query "${legacyKey}"`, { stdio: 'pipe' });
    } catch {
      continue; // 키가 없음. 지울 것도 없다
    }
    try {
      execSync(`reg delete "${legacyKey}" /f`, { stdio: 'pipe' });
      console.log(colorText(`✓ [migrate] 이전 레지스트리 키 삭제: ${legacyKey}`, 'green'));
    } catch (e: any) {
      console.warn(
        colorText(`[migrate] 이전 레지스트리 키 삭제 실패 (무시): ${e.message}`, 'yellow'),
      );
    }
  }
}
...
async function main(): Promise<void> {
  ...
  if (isGlobalInstall) {
    ...
    // v1.10.0: 이전 브랜딩의 스킬·호스트 등록 정리 (best-effort)
    cleanupLegacyInstall();
    printNextSteps(extDir);
  } else {
