...
4. 확인: `auto-chrome-mcp-bridge doctor` 가 10개 항목 전부 OK 인지 본다.
...
2. **전용 작업 창**. 꺼져 있으면 사용자 창에 백그라운드 탭을 만든다(탭은 활성화하지 않지만
   탭 목록이 늘어난다). 켜면 창이 분리된다.

예전 버전에서 토글을 껐던 사용자는 저장값이 그대로 존중되므로 새 기본값이 저절로 적용되지
않는다. 팝업의 **"무간섭 권장 설정으로 되돌리기"** 버튼이 한 번에 맞춰 준다.
...
```bash
auto-chrome-mcp-bridge doctor          # 상태 점검 (10개 항목)
